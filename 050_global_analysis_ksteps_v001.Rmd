---
title: "050_Global Variable Analysis: ksteps"
...
#
VARIABLE <- "ksteps"                   # Variable to be analyzed
#
VLABEL   <- "Steps walked"             # Label for variable
...
#
p <- search( key="ksteps", variable_names )          # Get variable columns number

if( is.na(p)) stop(                                  # Ensure variable columns was
    paste("Unable to find variable", VARIABLE))      # found ...
#
values <- six_months_walks[[p]]                      # Get variable vlues ...
#
...
plot_distribution( values, "ksteps", "Density histogram and plot" )
...
#
mean_value <- round( mean( values), 2)            # Get mean value and round to 2
                                                  # digits 

z  <- abs( qnorm(  ( 1 - probability )/ 2,        # Compute normal quantiles 
                    lower.tail = TRUE ) )         # for prediction intervals.

forecast <- rep(  mean_value, 
                  length( z ))

error_bound <- std * z                           # Compute prediction interval
                                                 # error bould.
...
We interpret the above tables as follows.  On a randomly selected walk the mean walking distance is 3.70 miles.  There is a 95% probability that the walk will be between 2.06 and 5.25 miles.  This may seem lioke a wide interval.  

This is due to some short walks I took six months ago which inflated the standard deviation of the error.  The length of different routes alks varies widely and this also increases the standard deviation.