...
#
VARIABLE  <- "ksteps"                    # Variables to be analyzed
VLABLE    <- "Steps(K)"                  # Variable label
 forecast <- "Thousand Steps"            # Variable unis
#
...
# functionality.  A script names load_paxkages_v003.R loads the
# packages I use frequebtly.
#######################################################################
#
...
file_path <- file.path( "data",                 # Build file path ...
                        "days_180.rds" )   
#
 load( file_path )                              # Get days_180 tibble ...            

days <-                                         # Keep only days when a walk
    days_180 %>%                                # occured ...
        filter( walked == 1 )                   
...
## Univariate analysis for `r VLABLE` walked

The following is an analysis miles walked variable.

```{r extract_variable_vaalues}

variable_names <- colnames( days )                 # Get variable names vector ...
#
p <- search( key=VARIABLE,                         #  Determine variable column 
             variable_names )                      # number ...

###############################################################################
# If the variable number is missing whe issue an error message and stop.
###############################################################################
                                                   
if( is.na(p)) stop(                                # Ensure variable columns was
    paste("Unable to find variable", VARIABLE))    # found ...
#
values <- days[[p]]                                # Get variable values ...
#
values <- values[!is.na(values)]                   # Ensure that there ar no missing
                                                   # values in values ...
```  
...
plot_distribution( values,                       # Plot density historgram and
                   VARIABLE,                     # diagram
                   "Density Histogram and plot" )

```

### Normal quantile-quantile plot of miles

```{r draw_variable_qq_plot}

plot_qq_norm( values, variable_name =VLABLE )
...
ybar         <- boot.out$t[,1]                   # Get bootsrap means ...
std          <- boot.out$t[,2]                   # get boot strap standard
                                                 # deviation ...
#
bsample      <- tibble( ybar,std )               # Make tibble for use by 
                                                 # ggplot ...
#
p1a <-                                
    plot_distribution( ybar,                     # Draw density histogram and 
                       "Means distribution",     # density plot of the sample 
                       "Bootstrap Histogram" )   # means and 
                                                 # standard deviations ...
...
p1c <- 
    plot_qq_norm( ybar,                          # Draw normal qq plots of bootstrap
                R = 100,                         # means and standard deviations ...
                variable_name="Means",
...
```{r plot_two_bootstrap_means_vs_std}
ggplot( bsample,
        aes(x=ybar, y=std )) +
    geom_point() +
...
### Correlation of bootstrap estimates 

If the bootstrapped means and standard errors are normally  is distributed then they be uncorrelated.  We compute the correlation matrix to investigate if the correlation between the means and standard deviations are un-correlated.

```{r correlation}
#
cor.test( bsample$ybar, bsample$std ) 
#
```
...
lower       <-                                   # Compute ulower and upper 
    round( mean_value - error_bound, 2)          # bounds ...
upper       <- 
    round( mean_value + error_bound, 2)          

probability  <- round( 100 * probability, 0  )    # Convert decimal to precentage for                                                   # display 

predictions <-                                    # Build tibble for flextable ...
    tibble( probability,    
            error_bound,
            lower,
            forecast,
...
Here we display the prediction intervals for **`r VLABLE`**.  Unlike classical confidence intervals these are true probability statements.  
...
predictions %>% 
    flextable() %>% 
        set_header_labels( "probability" = "Probability",
                           "error_bound" = "Error\nbound",
                           "lower"       = "Lower\nlimit",
                           "forecast"    = "Point\nprediction",
                           "upper"       = "Upper\nlimit" )
...
We interpret the above tables as follows.  On a randomly selected walk the pedicted `r VLABLE` distance 'r prediction`  ``.  There is a wal95% probability that the walk will be between  and 
miles.  

This may seem like a wide interval. This is due to some short walks I took six months ago which inflated the standard deviation of the error.  The length of different routes alks varies widely and this also increases the standard deviation. label
