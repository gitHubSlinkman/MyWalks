...
#
recent_walk_days <-                              # Filter out days where 
    recent_walk_days %>%                         # is missing 
        filter( !is.na(miles) )
...
missing <-                                       # Make a tibble that contians rows
    recent_days %>%                             # where ksteps is missing
        filter( is.na(ksteps))

not_missing <-                                   # Make a tibble that contians rows
    recent_days %>%                             # where ksteps is not missing ...
        filter( !is.na(ksteps))
...
There are `r dim( imputed_days )[1]` days in the imputed_walks file.
