...
source('~/R-Projects/MyWalks/functions/report_summary.R')
source('~/R-Projects/MyWalks/functions/summarise_zones.R')
```
...
Note: The walks are sorted in reverse chronlogical order.  
...
### Zone minutes barchart

This barchart shows the number of beats in each heart rate zone.

```{r compute_zones}

summary <- summarize_zones( last_007_days_walks )

  summary %>% 
    mutate( percent = round( 100 * hours / sum(hours),  2 )) %>% 
      flextable() %>% 
        width( j = 1, 1.00 ) %>% 
        colformat_num( j = 2:3,
                       digits = 2 ) %>% 
        set_header_labels( range   = "Heart beats\nrange",
                           hours   = "Hours in\nzone",
...
### PLot Hear rate zones

```{r plot_heart_rate_zones}
