...
last_030_day_routes <- filter_by_days( all_day_routes, 31 )
...
compute_means( last_007_day_routes ) %>% 
flextable()
```
...
compute_means( last_030_day_routes ) %>% 
    flextable()
```

### Last_180_day_routes  
```{r test_last_180_days_routes}
compute_means( last_180_day_routes ) %>% 
    flextable()
```
```
...
compute_means( last_year_day_routes ) %>% 
    flextable()
```
