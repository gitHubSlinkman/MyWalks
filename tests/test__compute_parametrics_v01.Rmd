...
source('D:/R-Projects/MyWalks/functions/compute_parametrics_v01.R')
```
...
compute_parametrics( last_007_day_routes ) %>% 
flextable()
```
...
compute_parametrics( last_030_day_routes ) %>% 
    flextable()
```

### Last_180_day_routes  
```{r test_last_180_days_routes}
compute_parametrics( last_180_day_routes ) %>% 
    flextable()
```
```
...
compute_parametrics( last_year_day_routes ) %>% 
    flextable()
```
