...
diagnostics <- compute_diagnostics( fit1 )
...
### Remove outliers and pathological influential observarion

```{r remove outliers_and_pathologicsl_infliential_obervations}

obs <- 
    complete_data %>% 
        pull( obs )
observations <- tibble( obs )
...
cleansed_obs <- 
    dplyr::setdiff( observations, deletions )

cleansed_data <- 
    left_join( cleansed_obs,
               complete_data,
               by = "obs" )
...
###########################################################################
# Refit data modle with fut data 
###########################################################################

fit2 <- lm( steps ~ miles,
            data = cleansed_data )

models <- c( models, fit2 )
...
plot_qq_norm( variable = residuals( fit2 ),
              variable_name = "Residuals",
              plot_title = "Normal QQ plot: fit2" )
```
