...
source('~/R-Projects/MyWalks/functions/report_fit_summary.R')
source('~/R-Projects/MyWalks/functions/report_fit_coefficients.R')
source('~/R-Projects/MyWalks/functions/report_diagnostics.R')
...
```{r list errorsresons_for missing_walks}
...
report_fit_summary( fit1 )


```
 
### Regression coefficient summary 
```{r report_regression_coefficients}

report_fit_coefficients( fit1 )
...
diagnose_heteroskedasticity( fit1 )
...
plot_qq_norm( variable = residuals( fit1 ),
...
########################################################################
# Compute the standard diagobstics for linear regression.  We pass 
# the case ientifier so we can ties the diagnostics back to the orginal
# data.
#########################################################################
...
diagnostics <- compute_diagnostics( id = id,
                                    fit1 )

###########################################################################
# Build a combined table of the completed data and the regression 
# diagnostics by a natural join.
###########################################################################

complete_data_diagnostics <- 
  inner_join( complete_data, 
              diagnostics,
              by = "id" )

report_diagnostics( diagnotics )

```

## Set cases with outliers and significant dffits to $NA$

We se the cases with significant $residuals$ and high $dffit$ and Cook's distiace to NA.  The creireia we will use are:

* For residuals we will use the Bonferroni family leval of 0.01
* For dffits we will use a value of $3\ sqrt{p/n}$ 
* For Cook's distance we will use a value of $9/n$.

```{r find_bad_data_obervations}

###########################################################################
# Compute the critical value for a 1% level of significance
##########################################################################

n   <- length( residuals( fit1 ))
p   <- length( coefficients( fit1 ))
dfe <- n - p
tcv <- abs( qt( 0.01 / (2 * n ), dfe ))


###########################################################################
# We detected all observations where the studentized rsiduals is greater
# than 1% critical value of the Bonferroni Student's t distribution.
###########################################################################
...
##########################################################################
# Flag type of bad data.
##########################################################################

m <- dim(  detected_outliers )[1]
type <- rep( "outlier", m )
...
##########################################################################
# Find observations where the dffits exceeds three times the standard
# of the mean function.
##########################################################################

detected_dffits <- 
  diagnostics %>% 
    filter( abs( dffits ) > criteria ) %>% 
      select( id )
m <- dim(  detected_dffits )[1]
type <- rep( "dffits", m )
...
#########################################################################
# Find observations where Cook's distance exceeds 9 divided by 
# the sample size
#########################################################################
...
steps <- na_if( steps, steps )
steps


cleansed_data <- 
  dirty_data %>% 
    select( id:end_steps, hours:type) %>% 
      add_column( steps, .after = "end_steps" )
...
## Fit model with cleansed_data

```{r fit_model_with cleansed_data}

###########################################################################
# Fit the model with the cleansed data and report it.
###########################################################################

fit2 <- lm( steps ~ miles,
            data = clean_data )

report_fit_coefficients( fit2 )

report_fit_summary( fit2 )

```

### Model 2 Diagnostoc plots

```{r diagnose_fit2}


diagnose_linearity( fit2 )
...
diagnose_heteroskedasticity( fit2 )
