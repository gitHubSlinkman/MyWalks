...
###########################################################################
# Wee will only use data where both the the miles and steps are both 
...
diagnose_heteroskedasticity( slr )

```

## Normal Quantile-Quantile plot pf the residuals

This plot is used to assess the normality of the residuals which are the estimators of the underlying error terms.

If the quantile-qantile plot is within the simulated error bounds then we can't conclude that the residuals are not normally distributed.
...
The noemal quantile-quantile plot can also be used to detect heteroskedasticity and outliers.

```{r assess_non-normality}

plot_qq_norm( variable = residuals( slr ),
...
### Outliers, Influence measures, and other  diagnostics

```{r compute_hat_values}

###########################################################################

############################################################################ Create diagnostoc repoert and display it.  We first obtsin the id of
# the sample data an
###########################################################################
...
###########################################################################
# We compute the obaservation diagnostics for every obswervation ...
###########################################################################


diagnostics <- 
    compute_diagnostics( id = date_time, 
                         slr )
```
