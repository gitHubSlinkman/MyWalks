...
```{r load_functions}
#
source('~/R-Projects/Rsource/load_packages_v003.R')        # Load my favvorite 
                                                           # packages ...
source('~/R-Projects/Rsource/compute_dates_v004.R')        # To compute dates ...
source('functions/display_selected_days_v001.R')           # To display days  ...
#
```
...
### Display first 7 days in last 180 days 
 

```{r display_first_10}
#
first_7_days <- days_180[ 1:7, c(1,2,6:14)]         # Select first 7 days ...
#                                                     
display_selected_days( first_7_days )                # Display first 7 days 
#
```
 
 
#### Last 7 days of the last 180 days 

```{r display_last_10_days}
#
to <- dim( days_180 )[1]                      # Get subscript of last row...
from <- to - 7 + 1                           # Compute starting subscrupt of last
                                              # 10 rows ...
#
last_7_days <-                               # Get last 10 days ...
    days_180[ from:to, c(1,2,6:14)] 
#
display_selected_days( last_7_days )                 # Display as flextable ...
#
```
...
walk_days %>% 
    ggplot( aes( x = device )) +
       geom_bar( color="black",
            ...
            geom_text( stat='count', 
                        aes( label=..count.. ), vjust=-1) +
            coord_cartesian(ylim = c(0, max_bin + 10 )) +
            xlab( "Measure device" ) +
            theme_cowplot() +
            theme(axis.text.x=element_text(angle = -90, hjust = 0 ))
#       
...
shoe_bin <-                                    # Tabulate bins by shoes ...
    walk_days %>% 
        count( shoes )
#
max_count <- max( shoe_bin$n )                 # Compute neximum bin count
#
y_breaks <- seq( from = 0, 
               to   = max_count+25,
               by = 25 )
...
walk_days %>% 
    ggplot( aes( x = shoes )) +
       geom_bar( color="black",
            ...
            geom_text( stat='count', 
                        aes( label=..count.. ), vjust=-1) +
            coord_cartesian(ylim = c(0, max_bin + 25 )) +
            xlab( "Shoes" ) +
            theme_cowplot()
#  
```
