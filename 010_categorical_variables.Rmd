...
#
use_Imputed_missing_values <- TRUE               # Shoud this script read the imputed
...
source( "functions/compute_totals_means_stds_v004.R" )     # Compute means and totals 
source( "functions/get_walks_v004.R" )                     # To get data ...
#
```
...
```{r determin_input_source}

if ( use_Imputed_missing_values ) {
     walks <- readRDS( "data/imputed_walks")
} else {
statement2
}
...
### Number of days walks made and walks missed

```{r plot_bar_chart_days_walks_made_and_missed}
#
y_breaks <- seq( from=0, to=200, by=20 )
...
y_axis <- seq(from=0, to=100, by=10 )
